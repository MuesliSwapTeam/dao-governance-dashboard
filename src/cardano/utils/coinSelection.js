...
export const CoinSelection = {
  ...
  randomImprove: (inputs, outputs, limit, preset = []) => {
    ...
    for (let i = 0; i < splitOutputsAmounts.length; i++) {
      ...
      let range = {}
      range.ideal = Value.new(BigNum.from_str("0"))
        .checked_add(splitOutputsAmounts[i])
        .checked_add(splitOutputsAmounts[i])
      range.maximum = Value.new(BigNum.from_str("0"))
    ...
    if (utxoSelection.remaining.length > 0) {
      ...
      let dataCost = DataCost.new_coins_per_byte(
        BigNum.from_str(protocolParameters.coinsPerByte),
      )
      ...
      let minAmount = Value.new(min_ada_for_output(transactionOutput, dataCost))

      // This approximation simply doesn't work in most cases
      // let minAmount = Value.new(BigNum.from_str('1800000'))
...
function improve(utxoSelection, outputAmount, limit, range) {
  ...
  const newAmount = Value.new(BigNum.from_str("0"))
...
function mergeOutputsAmounts(outputs) {
  let compiledAmountList = Value.new(BigNum.from_str("0"))
...
function splitAmounts(amounts) {
    ...
    for (let i = 0; i < mA.keys().len(); i++) {
      ...
      for (let j = 0; j < mA.get(scriptHash).keys().len(); j++) {
        ...
        _assets.insert(
          ...
          BigNum.from_bytes(mA.get(scriptHash).get(assetName).to_bytes()),
        )
        ...
        let _value = Value.new(BigNum.from_str("0"))
  ...
  splitAmounts.push(Value.new(BigNum.from_bytes(amounts.coin().to_bytes())))
  ...
  if (minUTxOValue && BigInt(outputAmount.coin().to_str()) > 0) {
    ...
    if (compare(outputAmount, minAmount) < 0) {
      amount = minAmount.checked_add(
        Value.new(BigNum.from_str(protocolParameters.minUTxO)),
      )
    }
...
const cloneUTxOList = (utxoList) =>
  utxoList.map((utxo) => TransactionUnspentOutput.from_bytes(utxo.to_bytes()))
